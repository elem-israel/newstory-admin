...
export default function CommentForm() {
  ...
  return (
    <div
      className={`form ${isHide ? "hide" : ""} d-flex justify-content-center`}
    >
      {postToShow && (
        <ColoredCard
        ...
        >
          <React.div>
            ...
            <hr />
            <label htmlFor="freetext">ציטוטים וצירופי מילים</label>
            <textarea
              id="freetext"
              name="freetext"
            ...
            />
            <hr />
            <label>סיבת דירוג אנליסט</label>

            <MultiSelectPicker
              options={options.filter((op: any) => !pickedOptions.includes(op))}
              pickedValues={pickedOptions}
              onDrop={(value: string) =>
                setPickedOptions(pickedOptions.filter((op) => op != value))
              }
              onPick={(value: string) =>
                setPickedOptions([...pickedOptions, value])
              }
            />
            <hr />
            <label htmlFor="review">הערות אנליסט</label>
            <textarea
              id="review"
              name="review"
              className="form-control"
            ...
            <hr />
            <label>סיכון לפגיעה כללית</label>
            <GradePicker
              onPick={(val: any) => {
                const updatedPost = {
                  ...postToShow,
            ...
            />
            <hr />
            <label>סיכון לפגיעה מינית</label>
            <GradePicker
              plate="dark"
              onPick={(val: any) => {
                const updatedPost = {
